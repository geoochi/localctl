...
- 危险操作（卸载/禁用/删除）前端用 `window.confirm` 二次确认（见 ServiceCard.tsx 的 DANGEROUS_OPS），新增操作请保持此约定。
- 卡片按钮合并约定：Start+Restart 合并为单个「运行/重启」（kickstart -k 对未运行任务即启动）；Stop 与 Unload 同为 bootout，只保留「卸载」。API 仍保留 start/stop op 供外部客户端。
